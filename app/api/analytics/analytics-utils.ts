...
import type { ChartConfig, DataInsight, ChartDataPoint } from '@/app/chat/types';
 ...
 */
export async function parseExcel(base64Data: string): Promise<string[][]> {
...
export function detectColumnType(
  values: string[]
): 'number' | 'currency' | 'percent' | 'date' | 'text' {
 ...
 */
export function parseValue(val: string, type: string): number | null {
 ...
 */
export function calculateStats(values: number[]): ColumnInfo['stats'] {
  if (values.length === 0) {
 ...
 */
export function formatNumber(num: number, type: string = 'number'): string {
  if (type === 'currency') {
 ...
 */
export function generateInsights(columns: ColumnInfo[]): DataInsight[] {
 ...
 */
export function generateCharts(columns: ColumnInfo[], rows: string[][]): ChartConfig[] {
 ...
 */
export function generateSuggestions(columns: ColumnInfo[]): string[] {
