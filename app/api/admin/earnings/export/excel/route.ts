...
export async function GET(request: NextRequest) {
  try {
    ...
    csvRows.push(`Profit Margin,${totalRevenue > 0 ? ((totalProfit / totalRevenue) * 100).toFixed(2) : 0}%`);
    csvRows.push('');
    ...
    csvRows.push('Model,Usage Count,Input Tokens,Cached Input Tokens,Output Tokens,Live Search Calls,Total Cost');
    ...
    csvRows.push(`Total Tokens,${newsData?.reduce((sum: number, n: { tokens_used: number }) => sum + (n.tokens_used || 0), 0) || 0}`);
    ...
    users?.forEach((u: { id: string; email: string; full_name: string | null; subscription_tier: string }) => {
