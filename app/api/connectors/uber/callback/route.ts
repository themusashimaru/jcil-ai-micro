/**
 * UBER CALLBACK - Handle OAuth Callback
 * ======================================
 *
 * Handles the OAuth callback from Uber, exchanges the code for tokens,
...
import { exchangeCodeForTokens, getCurrentUser } from '@/lib/connectors/uber';
import { encrypt as encryptToken } from '@/lib/security/crypto';

const log = logger('UberCallback');
...
export async function GET(request: NextRequest) {
  ...
  const userId = cookieStore.get('uber_oauth_user')?.value;

  // Clear OAuth cookies
  cookieStore.delete('uber_oauth_state');
  cookieStore.delete('uber_oauth_user');

  // Check for errors from Uber
  if (error) {
    log.error('Uber auth error', { error });
  ...
  try {
    ...
    const uberUser = await getCurrentUser(tokens.access_token);
    log.info('Uber user profile', { uberId: uberUser.uuid, name: uberUser.first_name });
    ...
    const { error: updateError } = await adminClient
      .from('users')
      .update({
        uber_access_token: encryptedAccessToken,
        uber_refresh_token: encryptedRefreshToken,
        uber_token_expires_at: expiresAt,
        uber_user_id: uberUser.uuid,
        uber_first_name: uberUser.first_name,
        uber_last_name: uberUser.last_name,
        uber_email: uberUser.email,
        uber_connected_at: new Date().toISOString(),
      })
      .eq('id', userId);
    ...
    log.info('Uber connected successfully', { userId, uberId: uberUser.uuid });

    // Redirect to settings with success
    return NextResponse.redirect(
      new URL('/settings?tab=connectors&uber=connected', request.url)
    );
  } catch (err) {
