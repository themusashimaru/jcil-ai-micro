...
export default function UsersPage() {
    ...
    return (
      <div className="min-h-screen p-8 bg-background">
        <div className="flex h-64 items-center justify-center">
          <div className="text-xl text-text-muted">Loading users...</div>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen p-8 bg-background">
        <div className="glass-morphism rounded-2xl p-6">
          ...
          <div className="flex gap-3">
            <button
              onClick={fetchUsers}
              className="rounded-lg px-4 py-2 text-white hover:opacity-90 bg-primary"
            >
              Retry
            </button>
            <button
  ...
  return (
    <div className="min-h-screen p-8 bg-background text-text-primary">
      <div className="mb-8 flex items-center justify-between">
        <h1 className="text-3xl font-bold">User Management</h1>
        <button
          onClick={fetchUsers}
          className="rounded-lg px-4 py-2 text-sm text-white hover:opacity-90 bg-primary"
        >
          Refresh
        </button>
      </div>
      ...
      {stats && (
        <div className="mb-8 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
          <div className="glass-morphism rounded-2xl p-6">
            <div className="mb-2 text-sm text-text-secondary">Total Users</div>
          ...
          <div className="glass-morphism rounded-2xl p-6">
            <div className="mb-2 text-sm text-text-secondary">Messages Today</div>
          ...
          <div className="glass-morphism rounded-2xl p-6">
            <div className="mb-2 text-sm text-text-secondary">Paid Subscriptions</div>
            <div className="text-3xl font-bold">
              {stats.usersByTier.basic + stats.usersByTier.pro + stats.usersByTier.executive}
            </div>
            <div className="mt-2 text-xs text-text-muted">Active: {stats.usersByStatus.active}</div>
          </div>

          <div className="glass-morphism rounded-2xl p-6">
            <div className="mb-2 text-sm text-text-secondary">Active Today</div>
            <div className="text-3xl font-bold">{stats.activeUsers.today}</div>
            <div className="mt-2 text-xs text-text-muted">30d: {stats.activeUsers.last30Days}</div>
          </div>
        </div>
      )}
        ...
        <div className="mb-8 glass-morphism rounded-2xl p-6">
          ...
          <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
            ...
            <div>
              ...
              <div className="text-sm text-text-secondary">Plus ($18/mo)</div>
            </div>
            <div>
              <div className="text-2xl font-bold text-purple-500">{stats.usersByTier.pro}</div>
              <div className="text-sm text-text-secondary">Pro ($30/mo)</div>
            </div>
            <div>
              <div className="text-2xl font-bold text-amber-500">{stats.usersByTier.executive}</div>
              <div className="text-sm text-text-secondary">Executive ($99/mo)</div>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="mb-6 glass-morphism rounded-2xl p-6">
        <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
          <input
            ...
            className="rounded-lg px-4 py-2 focus:outline-none bg-glass border border-theme text-text-primary"
          />
      ...
      {users.length === 0 && !loading && !error && (
        <div className="mb-6 glass-morphism rounded-2xl p-6">
          ...
          <div className="text-text-secondary">
            The API is working correctly, but there are no users in the database yet. This is normal
      ...
      <div className="lg:hidden space-y-4">
          ...
          <div key={user.id} className="glass-morphism rounded-xl p-4">
            <div className="flex items-start justify-between mb-3">
              <div className="flex-1 min-w-0">
                ...
                <div className="text-sm truncate text-text-secondary">{user.email}</div>
              </div>
              <div className="flex gap-2 ml-2">
                <span
                  className={`inline-block rounded-full px-2 py-1 text-xs font-semibold ${getTierColor(user.subscription_tier || 'free')}`}
                >
            ...
            <div className="grid grid-cols-2 gap-3 mb-4 text-sm">
              ...
              <div>
                ...
                <span className="ml-1 font-medium">
                  {(user.total_images || 0).toLocaleString()}
                </span>
              </div>
              <div>
            ...
            <a
              ...
              className="block w-full text-center rounded-lg px-4 py-3 text-sm font-medium text-white hover:opacity-90 transition bg-primary"
            >
              View Chats
            </a>
          </div>
        ...
        {filteredUsers.length === 0 && users.length > 0 && (
          <div className="glass-morphism rounded-xl p-8 text-center text-text-muted">
      ...
      <div className="hidden lg:block glass-morphism rounded-2xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="border-b border-theme bg-glass">
              <tr>
            ...
            <tbody className="border-t border-theme">
              {filteredUsers.map((user, index) => (
                <tr
                  key={user.id}
                  className={`hover:bg-black/5 dark:hover:bg-white/5 ${index < filteredUsers.length - 1 ? 'border-b border-theme' : ''}`}
                >
                  ...
                  <td className="px-6 py-4">
                    <span
                      className={`inline-block rounded-full px-3 py-1 text-xs font-semibold ${getTierColor(user.subscription_tier || 'free')}`}
                    >
                      {(user.subscription_tier || 'free').charAt(0).toUpperCase() +
                        (user.subscription_tier || 'free').slice(1)}
                    </span>
                  ...
                  <td className="px-6 py-4">
                    {user.subscription_status ? (
                      <span
                        className={`inline-block rounded-full px-3 py-1 text-xs font-semibold ${getStatusColor(user.subscription_status)}`}
                      >
                  ...
                  <td className="px-6 py-4 text-right">
                    ...
                    <div className="text-sm text-text-secondary">
                      {(user.total_messages || 0).toLocaleString()} total
                    </div>
                  </td>
                  <td className="px-6 py-4 text-right">
                    <div className="font-medium">{user.images_generated_today || 0} today</div>
                    <div className="text-sm text-text-secondary">
                      {(user.total_images || 0).toLocaleString()} total
                    </div>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4 text-center">
                    <a
                      href={`/admin/users/${user.id}`}
                      className="inline-block rounded-lg px-4 py-2 text-sm text-white hover:opacity-90 transition bg-primary"
                    >
                      View Chats
                    </a>
                  </td>
