/**
 * BYOK (Bring Your Own Key) — Shared module
 *
 * Retrieves and decrypts user-provided API keys from their
 * `user_provider_preferences.provider_api_keys` storage.
 * Used by both the main chat route and the CodeLab chat route.
 */

import { safeDecrypt as decryptToken } from '@/lib/security/crypto';
import { logger } from '@/lib/logger';

const log = logger('BYOK');
...
export async function getUserBYOKConfig(
  ...
  try {
    ...
    if (!prefs?.provider_api_keys) return null;
    ...
    if (!decryptedKey) {
      // CRITICAL: Do NOT return null here — the caller would silently fall back
      // to the platform API key, meaning the user's key is ignored without notice.
      // Throw so callers can surface a meaningful error to the user.
    ...
    return { apiKey: decryptedKey, model: customModel };
  } catch (error) {
