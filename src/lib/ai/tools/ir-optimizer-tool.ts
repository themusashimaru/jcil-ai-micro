...
// Optimization passes
const OPTIMIZATION_PASSES: Record<
  string,
  {
    name: string;
    description: string;
...
export const iroptimizerTool: UnifiedTool = {
  name: 'ir_optimizer',
  description:
    'Intermediate Representation optimization - SSA conversion, CFG analysis, dead code elimination, constant folding, and more',
  parameters: {
    type: 'object',
    properties: {
      operation: {
...
`;
  ...
  return {
    ...
    eliminated: eliminated.map(
      (i) => `${i.dest} = ${i.opcode.toLowerCase()} ${i.operands.join(', ')}`
    ),
    statistics: {
    ...
    explanation:
      'Dead code elimination removes instructions whose results are never used downstream',
  };
}
...
`;
    ...
    if (instr.dest && values.every((v) => v !== null)) {
      ...
      switch (instr.opcode) {
        ...
        case 'MUL':
          ...
          break;
        case 'DIV':
          result = Math.floor(values[0]! / values[1]!);
          break;
        case 'MOD':
          result = values[0]! % values[1]!;
          break;
  ...
  return {
    ...
    explanation:
      'Constant folding evaluates expressions with known constant values at compile time',
  };
}
...
function analyzeDataflow(_args: Record<string, unknown>): Record<string, unknown> {
  ...
  const liveness: Record<
    string,
    { liveIn: string[]; liveOut: string[]; def: string[]; use: string[] }
  > = {
    entry: {
      liveIn: ['x', 'y', '%cond'],
      liveOut: ['%a', '%b', '%c'],
    ...
    recommendations:
      level === 'O3'
        ...
        : level === 'Os' || level === 'Oz'
          ...
          : ['Good balance of speed and compile time', 'Suitable for most applications'],
  };
}
