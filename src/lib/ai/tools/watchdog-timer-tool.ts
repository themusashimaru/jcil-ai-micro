...
class WatchdogTimerSystem {
  ...
  kick(watchdogId: string): {
    ...
    stats.averageInterval =
      (stats.averageInterval * (stats.totalKicks - 1) + intervalMs) / stats.totalKicks;
  ...
  analyzeTiming(watchdogId: string): {
    ...
    if (jitterUs > 100) {
      recommendations.push(
        `High timing jitter (${jitterUs.toFixed(1)}us). Consider adjusting prescaler for better resolution.`
      );
    }

    if (config.type === 'standard' && config.timeoutMs < 100) {
      recommendations.push(
        'Very short timeout. Ensure system can reliably kick within this period.'
      );
    }
...
export const watchdogtimerTool: UnifiedTool = {
  name: 'watchdog_timer',
  description:
    'Full watchdog timer system with multiple instances, window mode, cascade configuration, and comprehensive monitoring',
  parameters: {
    type: 'object',
    properties: {
      operation: {
          ...
          content: JSON.stringify(
            {
              ...
              statistics: {
                ...
                shortestInterval:
                  result.status!.statistics.shortestInterval === Infinity
                    ? 'N/A'
                    : `${result.status!.statistics.shortestInterval}ms`,
          ...
          content: JSON.stringify(
            {
              ...
              check: {
                ...
                timeUntilWindowOpens: result.check!.timeUntilWindowOpens
                  ? `${result.check!.timeUntilWindowOpens}ms`
                  : undefined,
                timeUntilWindowCloses: result.check!.timeUntilWindowCloses
                  ? `${result.check!.timeUntilWindowCloses}ms`
                  : undefined,
              },
              recommendation:
                result.check!.status === 'too_early'
          ...
          content: JSON.stringify(
            {
              examples: [
                ...
                },
                {
                  ...
                  call: {
                    operation: 'configure',
                    watchdogId: 'backup_wdt',
                    timeoutMs: 2000,
                    cascadeTarget: 'final_wdt',
                  },
                },
              ],
            },
...
export function iswatchdogtimerAvailable(): boolean {
  return true;
}
