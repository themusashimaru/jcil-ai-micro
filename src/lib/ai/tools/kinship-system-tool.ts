...
const KINSHIP_SYSTEMS: Record<KinshipSystem, KinshipTerminology> = {
  'Eskimo': {
    system: 'Eskimo',
    description: 'Distinguishes lineal relatives (parents, children) from collateral relatives (aunts, uncles, cousins)',
    characteristics: [
  ...
  'Hawaiian': {
    system: 'Hawaiian',
    description: 'The most classificatory system - merges all relatives of the same generation and sex',
    characteristics: [
  ...
  'Crow': {
    system: 'Crow',
    description: 'Mirror of Omaha - merges relatives across generations on father\'s side (patrilateral skewing)',
    characteristics: [
  ...
  'Iroquois': {
    system: 'Iroquois',
    description: 'Distinguishes parallel from cross relatives, merging parallel relatives with lineal',
    characteristics: [
    ...
    societies: ['Iroquois Confederacy', 'Many Native American groups', 'Dravidian societies in India'],
    diagram: [
...
const DESCENT_SYSTEMS: Record<DescentType, {
  description: string;
  inheritance: string;
  residence: string[];
  examples: string[];
...
// Marriage rules
const MARRIAGE_RULES: Record<string, {
  description: string;
  variations: string[];
  ...
  'endogamy': {
    ...
    social_functions: ['Maintains group boundaries', 'Preserves wealth/status', 'Cultural continuity'],
...
export const kinshipsystemTool: UnifiedTool = {
  name: 'kinship_system',
  description: 'Anthropological kinship analysis - descent systems, marriage rules, terminology systems, relationship calculations',
  parameters: {
    type: 'object',
    properties: {
      operation: {
        type: 'string',
        enum: ['descent', 'marriage', 'terminology', 'analyze', 'relationship', 'compare', 'info', 'examples'],
        description: 'Operation type'
      },
      system: { type: 'string', enum: ['Eskimo', 'Hawaiian', 'Sudanese', 'Omaha', 'Crow', 'Iroquois'], description: 'Kinship terminology system' },
      descent_type: { type: 'string', enum: ['patrilineal', 'matrilineal', 'bilateral', 'ambilineal', 'double'], description: 'Descent system type' },
      marriage_rule: { type: 'string', description: 'Marriage rule to analyze' },
      ego_sex: { type: 'string', enum: ['male', 'female'], description: 'Sex of ego (reference person)' },
      relationship_path: { type: 'string', description: 'Relationship path (e.g., "MBS" for Mother\'s Brother\'s Son)' },
...
function analyzeDescentSystem(args: Record<string, unknown>): Record<string, unknown> {
  ...
  const implications = {
    ...
    naming_conventions: descentType === 'patrilineal'
...
function analyzeMarriageRules(args: Record<string, unknown>): Record<string, unknown> {
  ...
  return {
    ...
    relationship_to_descent: {
      ...
      'cross_cousin_marriage': 'Common in unilineal systems, especially with Iroquois/Dravidian terminology',
    ...
    exchange_theory: marriageRule.includes('cousin') ? {
      type: marriageRule.includes('cross') ? 'Generalized or Restricted Exchange' : 'Direct Exchange',
...
function generateRelationshipTerms(system: KinshipSystem): Record<string, string> {
  const terms: Record<KinshipSystem, Record<string, string>> = {
    'Eskimo': {
      'F': 'Father', 'M': 'Mother', 'FB': 'Uncle', 'MB': 'Uncle',
      'FZ': 'Aunt', 'MZ': 'Aunt', 'B': 'Brother', 'Z': 'Sister',
      'FBC': 'Cousin', 'MBC': 'Cousin', 'FZC': 'Cousin', 'MZC': 'Cousin'
    },
    'Hawaiian': {
      'F': 'Father', 'M': 'Mother', 'FB': 'Father', 'MB': 'Father',
      'FZ': 'Mother', 'MZ': 'Mother', 'B': 'Brother', 'Z': 'Sister',
      'FBC': 'Brother/Sister', 'MBC': 'Brother/Sister', 'FZC': 'Brother/Sister', 'MZC': 'Brother/Sister'
    },
    'Sudanese': {
      'F': 'Father', 'M': 'Mother', 'FB': 'Father\'s Brother', 'MB': 'Mother\'s Brother',
      'FZ': 'Father\'s Sister', 'MZ': 'Mother\'s Sister', 'B': 'Brother', 'Z': 'Sister',
      'FBS': 'FBS', 'FBD': 'FBD', 'MBS': 'MBS', 'MBD': 'MBD', 'FZS': 'FZS', 'FZD': 'FZD', 'MZS': 'MZS', 'MZD': 'MZD'
    },
    'Omaha': {
      'F': 'Father', 'M': 'Mother', 'FB': 'Father', 'MB': 'Mother\'s Brother',
      'FZ': 'Father\'s Sister', 'MZ': 'Mother', 'B': 'Brother', 'Z': 'Sister',
      'FBC': 'Brother/Sister', 'MBC': 'Mother\'s Brother/Mother', 'FZC': 'Child', 'MZC': 'Brother/Sister'
    },
    'Crow': {
      'F': 'Father', 'M': 'Mother', 'FB': 'Father', 'MB': 'Mother\'s Brother',
      'FZ': 'Father\'s Sister', 'MZ': 'Mother', 'B': 'Brother', 'Z': 'Sister',
      'FBC': 'Brother/Sister', 'MBC': 'Child', 'FZC': 'Father/Father\'s Sister', 'MZC': 'Brother/Sister'
    },
    'Iroquois': {
      'F': 'Father', 'M': 'Mother', 'FB': 'Father', 'MB': 'Uncle',
      'FZ': 'Aunt', 'MZ': 'Mother', 'B': 'Brother', 'Z': 'Sister',
      'FBC': 'Brother/Sister', 'MBC': 'Cross-Cousin', 'FZC': 'Cross-Cousin', 'MZC': 'Brother/Sister'
    }
  };
...
function getClassificationType(system: KinshipSystem): Record<string, unknown> {
  const types: Record<KinshipSystem, { type: string; merging_level: string; bifurcation: boolean }> = {
...
function analyzeStructuralPrinciples(system: KinshipSystem): Record<string, unknown> {
  return {
    generation_principle: system === 'Hawaiian' ? 'Primary (all relatives of same generation merged)'
      : system === 'Sudanese' ? 'Important but with many distinctions'
    ...
    bifurcation_principle: ['Omaha', 'Crow', 'Iroquois', 'Sudanese'].includes(system)
      ...
      : 'Not significant',
    collaterality_principle: system === 'Eskimo' ? 'Primary - distinguishes lineal from collateral'
      : system === 'Sudanese' ? 'Fully distinguished'
        : 'Modified by merging',
    relative_age: 'May be present in some societies using this system',
    lineage_skewing: system === 'Omaha' ? 'Matrilateral skewing (across mother\'s line)'
      : system === 'Crow' ? 'Patrilateral skewing (across father\'s line)'
        : 'Not present'
  };
}
...
function calculateRelationship(args: Record<string, unknown>): Record<string, unknown> {
  ...
  return {
    ...
    path_in_other_systems: Object.fromEntries(
      Object.keys(KINSHIP_SYSTEMS).map(s => [s, generateRelationshipTerms(s as KinshipSystem)[path] || 'varies'])
    )
  };
}

function parseRelationshipPath(path: string): { steps: string[]; generation: number; lineage: string } {
  const steps: string[] = [];
  ...
  for (const char of chars) {
    switch (char) {
      case 'F': steps.push('Father'); generation++; lineage = 'paternal'; break;
      case 'M': steps.push('Mother'); generation++; lineage = lineage === 'paternal' ? 'paternal' : 'maternal'; break;
      case 'B': steps.push('Brother'); break;
      case 'Z': steps.push('Sister'); break;
      case 'S': steps.push('Son'); generation--; break;
      case 'D': steps.push('Daughter'); generation--; break;
      case 'H': steps.push('Husband'); break;
      case 'W': steps.push('Wife'); break;
    }
  }
...
function generateKeyDifferences(systems: KinshipSystem[]): string[] {
  ...
  if (systems.includes('Eskimo') && systems.includes('Hawaiian')) {
    differences.push('Eskimo distinguishes lineal/collateral; Hawaiian merges all same-generation relatives');
  }
...
function getInfo(): Record<string, unknown> {
  return {
    operation: 'info',
    tool: 'kinship_system',
    description: 'Anthropological analysis of kinship systems, descent rules, and marriage patterns',
    capabilities: [
    ...
    relationship_notation: {
      ...
      'C': 'Child (generic)', 'P': 'Parent (generic)'
    },
    key_concepts: [
