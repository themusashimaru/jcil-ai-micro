...
export function clearFramebuffer(fb: Framebuffer, clearColor: Color = color(0, 0, 0, 255)): void {
...
export function computeEdgeEquation(v0: Vec2, v1: Vec2): EdgeEquation {
  return {
    a: v0.y - v1.y,
    b: v1.x - v0.x,
...
export function evaluateEdge(edge: EdgeEquation, x: number, y: number): number {
...
export function interpolateAttribute<T extends { [K in keyof T]: number }>(
  bary: Vec3,
  a0: T,
  a1: T,
  a2: T
  ...
  for (const key of Object.keys(a0) as (keyof T)[]) {
    ...
    (result as any)[key] =
      (a0[key] as number) * bary.x + (a1[key] as number) * bary.y + (a2[key] as number) * bary.z;
  }
  return result;
}
...
function perspectiveCorrectInterpolation(bary: Vec3, w0: number, w1: number, w2: number): Vec3 {
  ...
  for (let y = minY; y <= maxY; y++) {
    for (let x = minX; x <= maxX; x++) {
      ...
      if (v0.texCoord && v1.texCoord && v2.texCoord) {
        texCoord = {
          x:
            v0.texCoord.x * correctedBary.x +
            v1.texCoord.x * correctedBary.y +
            v2.texCoord.x * correctedBary.z,
          y:
            v0.texCoord.y * correctedBary.x +
            v1.texCoord.y * correctedBary.y +
            v2.texCoord.y * correctedBary.z,
        };
      }
      ...
      if (v0.normal && v1.normal && v2.normal) {
        normal = {
          x:
            v0.normal.x * correctedBary.x +
            v1.normal.x * correctedBary.y +
            v2.normal.x * correctedBary.z,
          y:
            v0.normal.y * correctedBary.x +
            v1.normal.y * correctedBary.y +
            v2.normal.y * correctedBary.z,
          z:
            v0.normal.z * correctedBary.x +
            v1.normal.z * correctedBary.y +
            v2.normal.z * correctedBary.z,
        };
      }
...
function rasterizeTriangleScanline(
  ...
  for (let y = yStart; y <= yEnd; y++) {
    ...
    if (longX < shortX) {
      ...
      zRight = shortZ;
      colorLeft = longColor;
      colorRight = shortColor;
    } else {
      xLeft = shortX;
      xRight = longX;
      zLeft = shortZ;
      zRight = longZ;
      colorLeft = shortColor;
      colorRight = longColor;
    }
...
function generateColorVisualization(
  fb: Framebuffer,
  charWidth: number = 40,
...
export const rasterizerTool: UnifiedTool = {
  name: 'rasterizer',
  description:
    'Comprehensive triangle rasterization engine with scanline and barycentric algorithms, perspective-correct interpolation, and attribute interpolation',
  parameters: {
    type: 'object',
    properties: {
      operation: {
  ...
  try {
    ...
    switch (operation) {
              ...
              examples: [
                {
                  ...
                  params: {
                    ...
                    triangles: [
                      {
                        v0: {
                          position: { x: 50, y: 10, z: 0.5, w: 1 },
                          color: { r: 255, g: 0, b: 0, a: 255 },
                        },
                        v1: {
                          position: { x: 10, y: 90, z: 0.5, w: 1 },
                          color: { r: 0, g: 255, b: 0, a: 255 },
                        },
                        v2: {
                          position: { x: 90, y: 90, z: 0.5, w: 1 },
      ...
      case 'barycentric': {
        ...
        stats.fillRate =
          stats.boundingBoxArea > 0 ? stats.pixelsCovered / stats.boundingBoxArea : 0;
