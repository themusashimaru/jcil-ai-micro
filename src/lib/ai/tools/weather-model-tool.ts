 ...
 */
export function coriolisParameter(latitude: number): number {
 ...
 */
function liftedIndex(surfaceTemp: number, surfaceDewPoint: number, temp500: number): number {
...
function lclTemperature(tempK: number, dewPointK: number): number {
  ...
  return (
    tempK -
    (tempK - dewPointK) / (1 + (CONSTANTS.L_v * CONSTANTS.gamma_s) / (CONSTANTS.R_dry * tempK))
  );
}
...
function calculateStabilityIndices(state: AtmosphericState, location: Location): StabilityIndices {
  ...
  const sweat =
    12 * (dewPoint850 - 273.15) +
    20 * (tt - 49) +
    2 * state.windSpeed +
    125 * (Math.sin(((state.windDirection - 30) * Math.PI) / 180) + 0.2);
...
function numericalForecast(
  ...
  for (let h = 0; h <= hours; h += dt) {
    ...
    const saturationRatio =
      (newHumidity * saturationVaporPressure(newTemp)) /
      saturationVaporPressure(initialState.temperature);
    ...
    forecasts.push({
      ...
      feelsLike:
        Math.round((feelsLikeTemperature(newTemp, newHumidity, newWindSpeed) - 273.15) * 10) / 10,
...
function ensembleForecast(
  ...
  for (let t = 0; t < timeSteps; t++) {
    ...
    const tempSpread = Math.sqrt(
      temps.reduce((sum, t) => sum + (t - meanTemp) ** 2, 0) / temps.length
    );
    ...
    meanForecast.push({
      ...
      feelsLike:
        Math.round(
          (ensembleMembers.map((m) => m.forecast[t].feelsLike).reduce((a, b) => a + b, 0) /
            members) *
            10
        ) / 10,
      humidity: Math.round(
        ensembleMembers.map((m) => m.forecast[t].humidity).reduce((a, b) => a + b, 0) / members
      ),
      windSpeed:
        Math.round(
          (ensembleMembers.map((m) => m.forecast[t].windSpeed).reduce((a, b) => a + b, 0) /
            members) *
            10
        ) / 10,
      windDirection: Math.round(
        ensembleMembers.map((m) => m.forecast[t].windDirection).reduce((a, b) => a + b, 0) / members
      ),
      precipitation:
        Math.round(
          (ensembleMembers.map((m) => m.forecast[t].precipitation).reduce((a, b) => a + b, 0) /
            members) *
            10
        ) / 10,
      precipitationType: ensembleMembers[0].forecast[t].precipitationType,
      cloudCover: Math.round(
        ensembleMembers.map((m) => m.forecast[t].cloudCover).reduce((a, b) => a + b, 0) / members
      ),
      condition: ensembleMembers[Math.floor(members / 2)].forecast[t].condition,
      uvIndex: Math.round(
        ensembleMembers.map((m) => m.forecast[t].uvIndex).reduce((a, b) => a + b, 0) / members
      ),
      visibility:
        Math.round(
          (ensembleMembers.map((m) => m.forecast[t].visibility).reduce((a, b) => a + b, 0) /
            members) *
            10
        ) / 10,
    });
...
const exampleLocations: Record<string, Location> = {
  new_york: { latitude: 40.7128, longitude: -74.006, elevation: 10, name: 'New York City' },
...
export const weathermodelTool: UnifiedTool = {
  name: 'weather_model',
  description:
    'Weather prediction and atmospheric modeling including numerical weather prediction, ensemble forecasting, stability analysis, and atmospheric calculations.',
  parameters: {
    type: 'object',
    properties: {
      operation: {
        ...
        description:
          'Operation: forecast (numerical), ensemble, stability (indices), analyze conditions, calculate atmospheric properties, examples, or info',
      },
      model: {
        type: 'string',
      ...
      location_name: {
        type: 'string',
        description:
          'Named location: new_york, london, tokyo, denver, singapore, reykjavik, sahara',
      },
      initial_conditions: {
        type: 'object',
    ...
    if (operation === 'calculate') {
      if (!calculation) {
        return {
          toolCallId: id,
          content:
            'Error: Specify calculation type (dew_point, feels_like, density, potential_temp, pressure_altitude, saturation_pressure)',
          isError: true,
        };
      }
      ...
      switch (calculation) {
        case 'dew_point':
          if (!temperature || humidity === undefined) {
            return {
              toolCallId: id,
              content: 'Error: temperature and humidity required',
              isError: true,
            };
          }
        ...
        case 'feels_like':
          if (!temperature || humidity === undefined || wind_speed === undefined) {
            return {
              toolCallId: id,
              content: 'Error: temperature, humidity, and wind_speed required',
              isError: true,
            };
          }
        ...
        case 'density':
          if (!pressure || !temperature || humidity === undefined) {
            return {
              toolCallId: id,
              content: 'Error: pressure, temperature, and humidity required',
              isError: true,
            };
          }
        ...
        case 'potential_temp':
          if (!temperature || !pressure) {
            return {
              toolCallId: id,
              content: 'Error: temperature and pressure required',
              isError: true,
            };
          }
        ...
        case 'pressure_altitude':
          if (!pressure || !temperature || altitude === undefined) {
            return {
              toolCallId: id,
              content: 'Error: pressure, temperature, and altitude required',
              isError: true,
            };
          }
    ...
    if (operation === 'analyze') {
      const analysis = {
        ...
        current_conditions: {
          ...
          feels_like_C:
            feelsLikeTemperature(state.temperature, state.humidity, state.windSpeed) - 273.15,
    ...
    if (operation === 'stability') {
      ...
      const interpretation = {
        ...
        convection_potential:
          indices.CAPE > 1000 && indices.LI < -2 ? 'HIGH' : indices.CAPE > 500 ? 'MODERATE' : 'LOW',
      };
        ...
        content: JSON.stringify(
          {
            ...
            ensemble: {
              ...
              average_spread_C:
                ensembleResult.spread.reduce((a, b) => a + b, 0) / ensembleResult.spread.length,
            },
