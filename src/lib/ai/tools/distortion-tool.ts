...
export const distortionTool: UnifiedTool = {
  name: 'distortion',
  description: 'Distortion and overdrive',
  parameters: {
    type: 'object',
    properties: {
...
export async function executedistortion(toolCall: UnifiedToolCall): Promise<UnifiedToolResult> {
  ...
  try {
    ...
    const result = { operation: args.operation, tool: 'distortion', processed: true };
...
export function isdistortionAvailable(): boolean { return true; }
