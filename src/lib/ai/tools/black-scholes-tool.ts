...
export const blackscholesTool: UnifiedTool = {
  name: 'black_scholes',
  description: 'Black-Scholes options pricing - call/put prices, Greeks (delta, gamma, theta, vega, rho), implied volatility',
  parameters: {
    type: 'object',
    properties: {
      operation: {
        type: 'string',
        enum: ['price_call', 'price_put', 'greeks', 'implied_volatility', 'price_both', 'put_call_parity', 'info'],
      ...
      dividend: { type: 'number', description: 'Continuous dividend yield (q) as decimal (default: 0)' },
      market_price: { type: 'number', description: 'Market option price for implied volatility calculation' },
      option_type: { type: 'string', enum: ['call', 'put'], description: 'Option type for implied volatility' }
    },
    required: ['operation']
  }
};
 ...
 */
function callPrice(S: number, K: number, T: number, r: number, sigma: number, q: number = 0): number {
 ...
 */
function putPrice(S: number, K: number, T: number, r: number, sigma: number, q: number = 0): number {
...
function calculateGreeks(S: number, K: number, T: number, r: number, sigma: number, q: number = 0) {
  ...
  const charm = q * expQT * normalCDF(d1) - expQT * normalPDF(d1) *
...
function impliedVolatility(
  ...
  marketPrice: number, optionType: 'call' | 'put', q: number = 0
    ...
    switch (operation) {
      ...
      case 'implied_volatility': {
        ...
        const { iv, iterations, error } = impliedVolatility(
          spot, strike, time, rate, market_price, option_type, dividend
        );
        const theoreticalPrice = option_type === 'call'
      ...
      default:
        result = {
          ...
          examples: [
            { operation: 'price_both', spot: 100, strike: 105, time: 0.5, rate: 0.05, volatility: 0.25 },
            { operation: 'greeks', spot: 50, strike: 50, time: 0.25, rate: 0.03, volatility: 0.3 },
            { operation: 'implied_volatility', spot: 100, strike: 100, time: 1, rate: 0.05, market_price: 10, option_type: 'call' }
          ]
        };
    }
