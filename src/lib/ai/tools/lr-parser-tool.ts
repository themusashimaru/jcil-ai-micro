...
export const lrparserTool: UnifiedTool = {
  name: 'lr_parser',
  description: 'LR parser generator',
  parameters: {
    type: 'object',
    properties: {
...
export async function executelrparser(toolCall: UnifiedToolCall): Promise<UnifiedToolResult> {
  ...
  try {
    ...
    const result = { operation: args.operation, tool: 'lr-parser', verified: true };
...
export function islrparserAvailable(): boolean { return true; }
