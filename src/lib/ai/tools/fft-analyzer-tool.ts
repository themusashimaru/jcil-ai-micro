...
export const fftanalyzerTool: UnifiedTool = {
  name: 'fft_analyzer',
  description: 'FFT frequency analysis with Cooley-Tukey algorithm, windowing functions, and spectral analysis',
  parameters: {
    type: 'object',
    properties: {
      operation: {
        type: 'string',
        enum: ['fft', 'ifft', 'dft', 'spectrum', 'spectrogram', 'filter', 'convolve', 'correlate', 'generate', 'window', 'info', 'examples'],
      ...
      signal_complex: {
        ...
        description: 'Complex input signal'
      },
      ...
      signal_type: {
        type: 'string',
        enum: ['sine', 'cosine', 'square', 'sawtooth', 'triangle', 'chirp', 'noise', 'impulse', 'step'],
        description: 'Type of signal to generate'
      },
      frequency: { type: 'number', description: 'Signal frequency in Hz' },
      frequencies: { type: 'array', items: { type: 'number' }, description: 'Multiple frequencies for composite signals' },
...
// Window functions
function applyWindow(signal: number[], windowType: string, beta: number = 5, sigma: number = 0.4): number[] {
  const n = signal.length;
...
function generateWindow(n: number, windowType: string, beta: number = 5, sigma: number = 0.4): number[] {
...
function findPeaks(magnitude: number[], freqBins: number[], threshold: number = 0.1): Array<{ frequency: number; magnitude: number; bin: number }> {
    ...
    switch (operation) {
      case 'fft': {
        ...
        if (args.window_type && args.signal) {
          const windowed = applyWindow(args.signal, args.window_type, args.kaiser_beta, args.gaussian_sigma);
      ...
      case 'ifft': {
        ...
        return {
          ...
          content: JSON.stringify({
            ...
            reconstructed_signal: reconstructed.slice(0, Math.min(50, reconstructed.length)).map((c, i) => ({
              index: i,
              real: c.re.toFixed(6),
              imag: c.im.toFixed(6)
            })),
        ...
        return {
          ...
          content: JSON.stringify({
            ...
            dominant_frequencies: peaks.slice(0, 10).map(p => ({
              frequency: p.frequency.toFixed(2) + ' Hz',
              magnitude: p.magnitude.toFixed(4),
              power_db: (10 * Math.log10(p.magnitude * p.magnitude / (Math.max(...mag) * Math.max(...mag)) + 1e-10)).toFixed(2) + ' dB'
            })),
            spectral_statistics: {
      ...
      case 'convolve': {
        ...
        const signal2: number[] = args.signal_complex?.map((c: Complex) => c.re) ||
      ...
      case 'generate': {
        ...
        const signal = generateSignal(signalType, sampleRate, duration, frequency, amplitude, frequencies);
      ...
      case 'window': {
        ...
        return {
          ...
          content: JSON.stringify({
            ...
            available_windows: ['rectangular', 'hamming', 'hanning', 'blackman', 'bartlett', 'kaiser', 'gaussian']
          }, null, 2)
        };
      }
      ...
      case 'info': {
        return {
          ...
          content: JSON.stringify({
            ...
            signal_types: ['sine', 'cosine', 'square', 'sawtooth', 'triangle', 'chirp', 'noise', 'impulse', 'step'],
