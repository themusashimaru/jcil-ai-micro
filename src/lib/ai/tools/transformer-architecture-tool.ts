 ...
 */
function layerNorm(x: Matrix, eps: number = 1e-5): { normalized: Matrix; mean: number[]; variance: number[] } {
...
function feedForward(
  ...
  const intermediate = x.map(row => {
    const expanded = new Array(dFF).fill(0).map((_, i) =>
      row.reduce((sum, v, j) => sum + v * Math.sin(i * j + 1), 0) / row.length
    );
  ...
  return intermediate.map(row => {
    return new Array(dModel).fill(0).map((_, i) =>
      row.reduce((sum, v, j) => sum + v * Math.cos(i * j + 1), 0) / row.length
    );
  });
}
...
function transformerBlock(
  ...
  const { output: attnOutput, attentionWeights } = multiHeadAttention(
    current, current, current, numHeads, dModel, mask
  );
...
const ARCHITECTURES: Record<string, ArchitectureConfig> = {
  ...
  'BERT-base': {
    ...
    vocabSize: 30522,
    maxSeqLen: 512,
    features: ['Bidirectional attention', 'Learned positional embeddings', 'Post-norm', 'MLM + NSP training'],
    parameters: '110M'
  },
  'T5-base': {
    name: 'T5 Base',
...
export const transformerarchitectureTool: UnifiedTool = {
  ...
  parameters: {
    ...
    properties: {
      operation: {
        type: 'string',
        enum: ['attention', 'positional', 'layer_norm', 'ffn', 'block', 'architecture', 'compare', 'info'],
        description: 'Operation to perform'
      },
      variant: {
        type: 'string',
        enum: ['GPT-2-small', 'GPT-2-medium', 'GPT-3', 'BERT-base', 'T5-base', 'LLaMA-7B', 'ViT-base'],
      ...
      input: {
        ...
        description: 'Input matrix for computation'
      },
      encoding_type: {
        type: 'string',
...
export async function executetransformerarchitecture(toolCall: UnifiedToolCall): Promise<UnifiedToolResult> {
  ...
  try {
    ...
    if (operation === 'positional') {
      ...
      if (encoding_type === 'sinusoidal') {
        ...
        encoding = {
          ...
          encoding: pe.slice(0, 5).map(row => row.slice(0, 8).map(v => Math.round(v * 1000) / 1000)),
          properties: {
        ...
        encoding = {
          ...
          cos: cos.slice(0, 4).map(row => row.slice(0, 4).map(v => Math.round(v * 1000) / 1000)),
          sin: sin.slice(0, 4).map(row => row.slice(0, 4).map(v => Math.round(v * 1000) / 1000)),
          properties: {
        ...
        encoding = {
          ...
          biases: biases.slice(0, 2).map(b => b.slice(0, 5).map(row => row.slice(0, 5).map(v => Math.round(v * 100) / 100))),
          properties: {
    ...
    if (operation === 'attention') {
      ...
      const inputMatrix = input || Array.from({ length: seq_len }, () =>
      ...
      const { output, attentionWeights } = multiHeadAttention(
        inputMatrix, inputMatrix, inputMatrix, num_heads, d_model, mask
      );
      ...
      return { toolCallId: id, content: JSON.stringify({
        ...
        output: {
          ...
          sample: output.slice(0, 3).map(row => row.slice(0, 4).map(v => Math.round(v * 1000) / 1000))
        },
        attentionPattern: {
          numPatterns: attentionWeights.length,
          head0Sample: attentionWeights[0]?.slice(0, 4).map(row =>
            row.slice(0, 4).map(v => Math.round(v * 100) / 100)
          )
        },
        explanation: {
    ...
    if (operation === 'layer_norm') {
      ...
      return { toolCallId: id, content: JSON.stringify({
        ...
        input: {
          ...
          sample: inputMatrix.slice(0, 2).map(row => row.slice(0, 4).map(v => Math.round(v * 100) / 100))
        },
        layerNorm: {
          formula: 'LN(x) = (x - mean) / sqrt(var + eps)',
          normalized: normalized.slice(0, 2).map(row => row.slice(0, 4).map(v => Math.round(v * 1000) / 1000)),
        ...
        rmsNorm: {
          ...
          normalized: rmsNormalized.slice(0, 2).map(row => row.slice(0, 4).map(v => Math.round(v * 1000) / 1000)),
    ...
    if (operation === 'ffn') {
      ...
      return { toolCallId: id, content: JSON.stringify({
        ...
        output: {
          shape: [output.length, output[0].length],
          sample: output.slice(0, 2).map(row => row.slice(0, 4).map(v => Math.round(v * 1000) / 1000))
        },
        explanation: {
