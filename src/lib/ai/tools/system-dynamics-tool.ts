 ...
 */
function evaluateEquation(equation: string, context: Map<string, number>): number {
  ...
  // Handle functions
  expr = expr.replace(/MIN\(([^,]+),([^)]+)\)/gi, (_, a, b) =>
    Math.min(Number(a), Number(b)).toString()
  );
  expr = expr.replace(/MAX\(([^,]+),([^)]+)\)/gi, (_, a, b) =>
 ...
 */
function runSimulation(model: SystemModel, config: SimulationConfig): SimulationResult {
    ...
    loops: [
      {
        ...
        type: 'reinforcing',
        description: 'More population → more births → more population',
      },
    ],
  },
  inventory: {
    ...
    flows: [
      ...
      },
      {
        ...
        from: 'Supply_Line',
        to: 'Inventory',
        equation: 'Supply_Line / delivery_delay',
        unit: 'units/week',
      },
      {
        name: 'Shipments',
        from: 'Inventory',
        to: null,
        equation: 'MIN(Desired_Shipments, Inventory / dt)',
        unit: 'units/week',
      },
    ],
    auxiliaries: [
    ...
    loops: [
      {
        ...
        type: 'balancing',
        description: 'Inventory control loop',
      },
    ],
  },
  sir: {
    ...
    flows: [
      ...
      },
      {
        ...
        from: 'Infected',
        to: 'Recovered',
        equation: 'Infected * recovery_rate',
        unit: 'people/day',
      },
    ],
    auxiliaries: [
    ...
    loops: [
      {
        name: 'R1',
        variables: ['Infected', 'Infection_Rate', 'Susceptible'],
        type: 'reinforcing',
        description: 'Contagion spread',
      },
      {
        name: 'B1',
        variables: ['Infected', 'Recovery', 'Recovered'],
        type: 'balancing',
        description: 'Recovery depletion',
      },
      {
        name: 'B2',
        variables: ['Susceptible', 'Infection_Rate', 'Infected'],
        type: 'balancing',
        description: 'Susceptible depletion',
      },
    ],
  },
  predator_prey: {
    ...
    flows: [
      ...
      },
      {
        ...
        from: null,
        to: 'Predators',
        equation: 'predation_efficiency * predation_rate * Prey * Predators',
        unit: 'animals/year',
      },
      {
        ...
        from: 'Predators',
        to: null,
        equation: 'Predators * predator_death_rate',
        unit: 'animals/year',
      },
    ],
    auxiliaries: [],
    ...
    loops: [
      {
        name: 'R1',
        variables: ['Prey', 'Prey_Births'],
        type: 'reinforcing',
        description: 'Prey reproduction',
      },
      {
        name: 'B1',
        variables: ['Prey', 'Predation', 'Predators', 'Predator_Births'],
        type: 'balancing',
        description: 'Predation control',
      },
      {
        name: 'R2',
        variables: ['Predators', 'Predator_Births', 'Prey', 'Predation'],
        type: 'reinforcing',
        description: 'Predator growth',
      },
    ],
  },
  project: {
    ...
    flows: [
      ...
      { name: 'Hiring', from: null, to: 'New_Staff', equation: 'hiring_rate', unit: 'people/week' },
      {
        name: 'Training',
        from: 'New_Staff',
        to: 'Experienced_Staff',
        equation: 'New_Staff / training_time',
        unit: 'people/week',
      },
    ],
    auxiliaries: [
      ...
      { name: 'Effective_Staff', equation: 'Experienced_Staff + New_Staff * 0.3', unit: 'people' },
      {
        ...
        equation: 'comm_factor * Total_Staff * (Total_Staff - 1) / 2',
        unit: 'hours/week',
      },
      {
        name: 'Productivity',
        equation:
          'MAX(0, Effective_Staff * base_productivity - Communication_Overhead * overhead_cost)',
        unit: 'tasks/week',
      },
    ],
    constants: {
    ...
    loops: [
      {
        ...
        type: 'balancing',
        description: 'Staffing up',
      },
      {
        name: 'B2',
        variables: ['Total_Staff', 'Communication_Overhead', 'Productivity'],
        type: 'balancing',
        description: 'Brooks Law overhead',
      },
    ],
  },
};
...
export const systemdynamicsTool: UnifiedTool = {
  ...
  description:
    'System dynamics modeling with stocks, flows, and feedback loops. Simulates complex systems over time using Forrester methodology.',
  parameters: {
    type: 'object',
    properties: {
      operation: {
        ...
        description:
          'Operation: simulate model, analyze behavior, create causal loop diagram, sensitivity analysis, examples, or info',
      },
      model_name: {
        type: 'string',
...
export async function executesystemdynamics(toolCall: UnifiedToolCall): Promise<UnifiedToolResult> {
  ...
  try {
    ...
    if (operation === 'info') {
      const info = {
        ...
        components: {
          ...
          feedback_loops:
            'Circular causal chains that amplify (R) or stabilize (B) system behavior',
        },
        loop_types: {
    ...
    if (operation === 'causal_loop') {
      if (!causal_links || !Array.isArray(causal_links)) {
        return {
          toolCallId: id,
          content:
            'Error: causal_links array required for loop analysis. Format: [{from, to, polarity}]',
          isError: true,
        };
      }
        ...
        content: JSON.stringify(
          {
            sensitivity_analysis: {
              ...
              results: {
                ...
                interpretation:
                  Math.abs(result.sensitivity) > 1
                    ? 'High sensitivity'
                    : Math.abs(result.sensitivity) > 0.5
    ...
    const sampledIndices = result.time
      .map((_, i) => i)
      .filter((i) => i % sampleRate === 0 || i === result.time.length - 1);
      ...
      content: JSON.stringify(
        {
          ...
          final_state: Object.fromEntries(
            Object.entries(result.stocks).map(([name, values]) => [
              name,
