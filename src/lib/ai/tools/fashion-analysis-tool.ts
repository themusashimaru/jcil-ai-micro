...
> = {
  '2024': [
    {
      trend: 'Quiet Luxury',
      description: 'Understated elegance with premium materials and minimal branding',
      keyItems: ['Cashmere', 'Silk', 'Tailored neutrals'],
      longevity: 'macro',
      demographics: ['25-55', 'Professionals'],
    },
    {
      trend: 'Cherry Red',
      description: 'Bold, saturated red across all categories',
      keyItems: ['Red blazer', 'Red bag', 'Red boots'],
      longevity: 'seasonal',
      demographics: ['18-40', 'Fashion-forward'],
    },
    {
      trend: 'Sheer Fabrics',
      description: 'Transparent and semi-sheer layering',
      keyItems: ['Mesh tops', 'Organza', 'Sheer dresses'],
      longevity: 'seasonal',
      demographics: ['18-35', 'Trend-setters'],
    },
    {
      trend: 'Maxi Everything',
      description: 'Floor-length skirts, dresses, and coats',
      keyItems: ['Maxi skirts', 'Long coats', 'Floor-length dresses'],
      longevity: 'macro',
      demographics: ['All ages'],
    },
    {
      trend: 'Soft Tailoring',
      description: 'Relaxed, unstructured suiting',
      keyItems: ['Soft blazers', 'Wide-leg trousers', 'Flowing fabrics'],
      longevity: 'macro',
      demographics: ['25-50', 'Professionals'],
    },
  ],
  '2023': [
    {
      trend: 'Barbiecore',
      description: 'Hot pink everything inspired by the Barbie movie',
      keyItems: ['Hot pink', 'Fuchsia', 'Pink accessories'],
      longevity: 'micro',
      demographics: ['18-35', 'Pop culture enthusiasts'],
    },
    {
      trend: 'Coastal Grandmother',
      description: 'Relaxed, Nancy Meyers-inspired coastal elegance',
      keyItems: ['Linen', 'Cream colors', 'Wicker bags'],
      longevity: 'seasonal',
      demographics: ['30-60', 'Classic style lovers'],
    },
    {
      trend: 'Y2K Revival',
      description: 'Early 2000s fashion comeback',
      keyItems: ['Low-rise jeans', 'Baby tees', 'Butterfly clips'],
      longevity: 'macro',
      demographics: ['16-30', 'Millennials/Gen Z'],
    },
  ],
};
...
export const fashionanalysisTool: UnifiedTool = {
  ...
  description:
    'Comprehensive fashion analysis - style matching, color coordination, body type recommendations, trend forecasting, wardrobe building',
  parameters: {
    type: 'object',
    properties: {
      operation: {
      ...
      style: {
        type: 'string',
        enum: ['classic', 'minimalist', 'bohemian', 'streetwear', 'romantic', 'edgy'],
        description: 'Style archetype',
      },
      season: {
        type: 'string',
        enum: ['spring', 'summer', 'autumn', 'winter'],
        description: 'Season for color/outfit recommendations',
      },
      body_type: {
        type: 'string',
      ...
      occasion: {
        type: 'string',
        enum: ['casual', 'business', 'formal', 'date_night', 'vacation', 'weekend'],
        description: 'Occasion for outfit',
      },
...
export async function executefashionanalysis(
...
function coordinateColors(args: Record<string, unknown>): Record<string, unknown> {
  ...
  const occasionColors = palette.filter(
    (c) => c.formality === occasion || c.formality === 'versatile'
  );
...
function calculateColorHarmonies(baseColor: string): Record<string, unknown> {
  ...
  const harmonies: Record<
    string,
    { complementary: string; analogous: string[]; triadic: string[] }
  > = {
    navy: {
      complementary: 'Burnt Orange',
      analogous: ['Teal', 'Purple', 'Royal Blue'],
      triadic: ['Rust', 'Sage'],
    },
    black: {
      complementary: 'White',
      analogous: ['Charcoal', 'Grey', 'Navy'],
      triadic: ['Red', 'White'],
    },
    burgundy: {
      complementary: 'Teal',
      analogous: ['Maroon', 'Wine', 'Plum'],
      triadic: ['Navy', 'Mustard'],
    },
    olive: {
      complementary: 'Plum',
      analogous: ['Sage', 'Forest Green', 'Khaki'],
      triadic: ['Rust', 'Navy'],
    },
    cream: {
      complementary: 'Navy',
      analogous: ['Beige', 'Ivory', 'Tan'],
      triadic: ['Brown', 'Dusty Rose'],
    },
    default: {
      complementary: 'Contrasting neutral',
      analogous: ['Adjacent colors'],
      triadic: ['Triangle colors'],
    },
  };
...
function analyzeBodyType(args: Record<string, unknown>): Record<string, unknown> {
  ...
  return {
    ...
    fit_guide: {
      tops:
        bodyType === 'pear'
          ...
          : bodyType === 'apple'
            ? 'Empire waist, flowy fabrics'
            : bodyType === 'inverted_triangle'
              ? 'V-necks, soft shoulders'
              : bodyType === 'rectangle'
                ? 'Peplum, ruffles, belted'
                : 'Fitted, emphasize waist',
      bottoms:
        bodyType === 'pear'
          ? 'A-line, dark colors'
          : bodyType === 'apple'
    ...
    patterns_and_prints: {
      flattering:
        bodyType === 'pear'
          ? ['Horizontal stripes on top', 'Solid bottoms']
          : bodyType === 'apple'
            ? ['Vertical stripes', 'V-patterns']
...
function analyzeWardrobe(args: Record<string, unknown>): Record<string, unknown> {
  ...
  const essentials = [
    {
      category: 'Tops',
      items: ['White button-down', 'Black t-shirt', 'Cashmere sweater', 'Silk blouse'],
    },
    {
      category: 'Bottoms',
      items: ['Dark jeans', 'Tailored trousers', 'Black skirt', 'Neutral chinos'],
    },
    {
      category: 'Outerwear',
      items: ['Trench coat', 'Blazer', 'Leather/denim jacket', 'Winter coat'],
    },
    {
      category: 'Shoes',
      items: ['White sneakers', 'Black heels/dress shoes', 'Ankle boots', 'Loafers'],
    },
    {
      category: 'Accessories',
      items: ['Quality belt', 'Classic watch', 'Structured bag', 'Versatile scarf'],
    },
  ];
  ...
  const gaps = essentials.map((category) => ({
    ...
    missing: category.items.filter(
      (item) => !existingPieces.some((p) => p.toLowerCase().includes(item.toLowerCase()))
    ),
    ...
    existingPieces.length > 0
      ? Math.round(
          (essentials
            ...
            essentials.flatMap((e) => e.items).length) *
            100
        )
      : 0;
    ...
    shopping_priorities: gaps
      ...
      .flatMap((g) =>
        g.missing.map((item) => ({
          ...
          estimated_cost:
            budget === 'luxury' ? '$200-500' : budget === 'moderate' ? '$50-150' : '$20-50',
        }))
      )
      .slice(0, 10),
    style_specific_additions: archetype.keyPieces
      .filter(
        (piece) =>
...
function buildOutfit(args: Record<string, unknown>): Record<string, unknown> {
  ...
  const outfitTemplates: Record<
    string,
    { top: string; bottom: string; shoes: string; outerwear: string; accessories: string[] }
  > = {
    casual: {
...
function buildCapsuleWardrobe(args: Record<string, unknown>): Record<string, unknown> {
  ...
  const totalPieces = Object.values(capsule)
    .flat()
  ...
  return {
    ...
    statistics: {
      ...
      investment_guide:
        budget === 'luxury' ? '$3,000-8,000' : budget === 'moderate' ? '$1,000-2,500' : '$300-800',
    },
    ...
    building_order: [
      {
        phase: 1,
        focus: 'Basics',
        items: ['White shirt', 'Dark jeans', 'Black trousers', 'White sneakers'],
      },
