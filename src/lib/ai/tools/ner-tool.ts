...
const ENTITY_PATTERNS: EntityPattern[] = [
  ...
  {
    type: 'MONEY',
    pattern: /\$\d{1,3}(?:,\d{3})*(?:\.\d{2})?(?:\s*(?:million|billion|trillion|m|b|k))?\b/gi,
    priority: 8,
  },
  {
    type: 'MONEY',
    pattern:
      /\b\d{1,3}(?:,\d{3})*(?:\.\d{2})?\s*(?:dollars?|euros?|pounds?|yen|USD|EUR|GBP|JPY)\b/gi,
    priority: 8,
  },
  ...
  {
    type: 'TIME',
    pattern: /\b\d{1,2}:\d{2}(?::\d{2})?\s*(?:AM|PM|am|pm|a\.m\.|p\.m\.)?\b/g,
    priority: 7,
  },
  ...
  // Ordinals
  {
    type: 'ORDINAL',
    pattern:
      /\b(?:first|second|third|fourth|fifth|sixth|seventh|eighth|ninth|tenth|\d+(?:st|nd|rd|th))\b/gi,
    priority: 5,
  },
  ...
  {
    type: 'QUANTITY',
    pattern: /\b\d+(?:\.\d+)?\s*(?:kg|km|m|cm|mm|lb|lbs|oz|ft|mi|mph|kmh|mb|gb|tb)\b/gi,
    priority: 6,
  },
];
...
function extractGazetteerEntities(
  text: string,
  _tokens: { token: string; start: number; end: number }[]
): Entity[] {
  const entities: Entity[] = [];
...
function extractRuleBasedEntities(
  _text: string,
    ...
    if (/^[A-Z][a-z]+$/.test(cleanToken) && !PERSON_TITLES.has(lowerToken)) {
      ...
      if (capitalizedTokens.length >= 1) {
        ...
        if (
          ...
          type = 'ORG';
          confidence = 0.85;
        } else if (capitalizedTokens.length === 1 && COMMON_FIRST_NAMES.has(lowerToken)) {
          type = 'PERSON';
          confidence = 0.5;
        }
...
export async function executener(toolCall: UnifiedToolCall): Promise<UnifiedToolResult> {
  ...
  try {
    ...
    if (operation === 'classify') {
      ...
      const exactMatch = entities.find((e) => e.text.toLowerCase() === text.toLowerCase());
