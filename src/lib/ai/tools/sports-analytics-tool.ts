...
interface AdvancedMetrics {
  ...
  trueShooting?: number;  // True Shooting %
  usageRate?: number;
...
function calculatePER(stats: {
  ...
  const factor = (2 / 3) - (0.5 * 0.3) / (2 * 0.44);
  const VOP = 1.0; // Simplified value of possession

  const uPER = (1 / stats.minutes) * (
    stats.points
    + 0.4 * stats.fgMade
...
/**
 * Calculate expected goals (xG) simplified model
 */
function calculateXG(shots: { distance: number; angle: number; header: boolean }[]): number {
  let totalXG = 0;

  for (const shot of shots) {
    // Distance factor (closer = higher xG)
    const distFactor = Math.max(0, 1 - shot.distance / 40);

    // Angle factor (central = higher xG)
    const angleFactor = Math.cos(shot.angle * Math.PI / 180);

    // Header penalty
    const headerFactor = shot.header ? 0.7 : 1.0;

    // Base xG calculation
    const xg = 0.1 + 0.3 * distFactor * angleFactor * headerFactor;
    totalXG += Math.min(0.9, xg);
  }
...
function calculateWAR(
  ...
  // Replacement level and scale vary by sport
  const replacementLevels: Record<Sport, number> = {
    basketball: 0.1,
    football: 0.05,
    soccer: 0.08,
    baseball: 0.03,
  ...
  const gamesPerSeason: Record<Sport, number> = {
    basketball: 82,
    football: 17,
    soccer: 38,
    baseball: 162,
  ...
  const replacement = replacementLevels[sport];
  const seasonGames = gamesPerSeason[sport];

  // WAR = (Value - Replacement) * Games / SeasonGames
  const war = (valueAdded - replacement) * gamesPlayed / seasonGames;
...
function predictMatch(
  ...
  const homePtDiff = (homeTeam.pointsFor - homeTeam.pointsAgainst) / (homeTeam.wins + homeTeam.losses);
  const awayPtDiff = (awayTeam.pointsFor - awayTeam.pointsAgainst) / (awayTeam.wins + awayTeam.losses);
  const ptDiffFactor = (homePtDiff - awayPtDiff) / 20;

  // Home record factor
  const homeAtHome = homeTeam.homeRecord.wins / (homeTeam.homeRecord.wins + homeTeam.homeRecord.losses) || 0.5;
  const awayOnRoad = awayTeam.awayRecord.wins / (awayTeam.awayRecord.wins + awayTeam.awayRecord.losses) || 0.5;

  // Combined probability
  let homeWinProb = (homeWinPct * 0.3 + homeAtHome * 0.2 + (0.5 + ptDiffFactor) * 0.3 + 0.5 * 0.2) + homeAdv;
  ...
  const homeScore = baseScore * (homeTeam.pointsFor / (homeTeam.pointsFor + homeTeam.pointsAgainst) * 2);
  const awayScore = baseScore * (awayTeam.pointsFor / (awayTeam.pointsFor + awayTeam.pointsAgainst) * 2);

  // Key factors
  const keyFactors: string[] = [];
  if (homeAtHome > 0.6) keyFactors.push(`${homeTeam.name} strong at home (${Math.round(homeAtHome * 100)}%)`);
  if (awayOnRoad > 0.5) keyFactors.push(`${awayTeam.name} good on road (${Math.round(awayOnRoad * 100)}%)`);
  if (Math.abs(ptDiffFactor) > 0.05) keyFactors.push(`Point differential favors ${ptDiffFactor > 0 ? homeTeam.name : awayTeam.name}`);
...
function analyzePerformance(
  ...
  const offense = offenseMetrics.length > 0
  ...
  // Efficiency
  const minutesRatio = stats.minutesPerGame / leagueAverages.minutesPerGame;
...
const leagueAverages: Record<Sport, PlayerStats> = {
  basketball: { name: 'League Avg', position: '', games: 65, minutesPerGame: 28, points: 15.5, assists: 3.2, rebounds: 5.5 },
  soccer: { name: 'League Avg', position: '', games: 30, minutesPerGame: 75, points: 8, assists: 4, goals: 8 },
  football: { name: 'League Avg', position: '', games: 14, minutesPerGame: 60, points: 0, assists: 0, passingYards: 3500, completions: 320, attempts: 500 },
  baseball: { name: 'League Avg', position: '', games: 140, minutesPerGame: 0, points: 0.26, assists: 0 }, // batting avg as points
  hockey: { name: 'League Avg', position: '', games: 70, minutesPerGame: 18, points: 45, assists: 25, goals: 20 }
};
...
export const sportsanalyticsTool: UnifiedTool = {
  ...
  description: 'Sports analytics including player performance analysis, team statistics, match predictions, and advanced metrics calculation (PER, WAR, xG, etc.).',
  parameters: {
    type: 'object',
    properties: {
      operation: {
        type: 'string',
        enum: ['player_stats', 'team_analysis', 'prediction', 'advanced_metrics', 'performance', 'compare', 'examples', 'info'],
        description: 'Operation: player_stats, team_analysis, prediction, advanced_metrics, performance rating, compare players/teams, examples, or info'
      },
      sport: {
        type: 'string',
...
export async function executesportsanalytics(toolCall: UnifiedToolCall): Promise<UnifiedToolResult> {
  ...
  try {
    ...
    const {
      operation, sport = 'basketball', player, player_name, home_team, away_team,
      detailed_stats
    } = args;
    ...
    if (operation === 'info') {
      const info = {
        ...
        metrics: {
          basketball: ['PER (Player Efficiency Rating)', 'True Shooting %', 'Usage Rate', 'Win Shares', 'VORP'],
          soccer: ['xG (Expected Goals)', 'xA (Expected Assists)', 'Pressing Rate', 'Pass Completion'],
    ...
    // Get player data
    const playerData = player_name && examplePlayers[player_name]
      ? examplePlayers[player_name]
      : player ? { stats: player, sport } : null;
    ...
    if (operation === 'advanced_metrics') {
      ...
      if (sportType === 'basketball' && detailed_stats) {
        ...
        const ts = trueShooting(detailed_stats.points, detailed_stats.fgAttempts, detailed_stats.ftAttempts);
        ...
        return {
          ...
          content: JSON.stringify({
            ...
            advanced_metrics: {
              ...
              PER_rating: per >= 25 ? 'MVP Level' : per >= 20 ? 'All-Star' : per >= 15 ? 'Average' : 'Below Average',
    ...
    if (operation === 'team_analysis') {
      ...
      const ptDiff = (teamData.pointsFor - teamData.pointsAgainst) / (teamData.wins + teamData.losses);
      const winPct = teamData.wins / (teamData.wins + teamData.losses) * 100;
      const homeWinPct = teamData.homeRecord.wins / (teamData.homeRecord.wins + teamData.homeRecord.losses) * 100;
      const awayWinPct = teamData.awayRecord.wins / (teamData.awayRecord.wins + teamData.awayRecord.losses) * 100;
      ...
      return {
        ...
        content: JSON.stringify({
          ...
          scoring: {
            points_per_game: Math.round(teamData.pointsFor / (teamData.wins + teamData.losses) * 10) / 10,
            points_allowed: Math.round(teamData.pointsAgainst / (teamData.wins + teamData.losses) * 10) / 10,
    ...
    if (operation === 'prediction') {
      ...
      return {
        ...
        content: JSON.stringify({
          ...
          pick: prediction.homeWinProb > prediction.awayWinProb ? prediction.homeTeam : prediction.awayTeam
        }, null, 2)
      };
    }
    ...
    return {
      toolCallId: id,
      content: 'Error: Invalid operation. Use player_stats, team_analysis, prediction, advanced_metrics, performance, or info',
      isError: true
    };
