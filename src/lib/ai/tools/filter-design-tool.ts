 ...
 */
function butterworthAnalogPrototype(order: number): { zeros: Complex[]; poles: Complex[]; gain: number } {
  const poles: Complex[] = [];
...
function bilinearTransform(
  ...
  const digitalZeros: Complex[] = [...analogZeros.map(z => {
...
function evaluateTransferFunction(b: number[], a: number[], z: Complex): Complex {
  ...
  for (let k = 0; k < Math.max(b.length, a.length); k++) {
    ...
    // z^(-k-1) = z^(-k) * z^(-1)
    const zInv: Complex = { re: z.re / (z.re * z.re + z.im * z.im), im: -z.im / (z.re * z.re + z.im * z.im) };
  ...
  try {
    ...
    if (operation === 'design') {
      ...
      if (design === 'fir') {
        switch (filter_type) {
          ...
          case 'bandstop':
            ...
            break;
          default:
            return { toolCallId: id, content: `Error: Unknown filter type '${filter_type}'`, isError: true };
        }
        ...
        const result = {
          ...
          frequencyResponse: {
            passbandRipple_dB: Math.max(...response.magnitudeDB.slice(0, Math.floor(cutoff * 64))) -
                               Math.min(...response.magnitudeDB.slice(0, Math.floor(cutoff * 64))),
            stopbandAttenuation_dB: -Math.min(...response.magnitudeDB.slice(Math.floor(cutoff * 64)))
          }
        };
      ...
      if (design === 'butterworth') {
        ...
        if (filter_type !== 'lowpass') {
          return { toolCallId: id, content: 'Note: Only lowpass Chebyshev implemented. Use Butterworth for other types.', isError: false };
        }
      } else {
        return { toolCallId: id, content: `Error: Unknown design method '${design}'`, isError: true };
      }
    ...
    if (operation === 'apply') {
      if (!signal || !Array.isArray(signal)) {
        return { toolCallId: id, content: 'Error: signal array required for apply operation', isError: true };
      }
      ...
      const filtered = zero_phase
        ? filtfilt(signal, b, a)
        : (a.length === 1 ? applyFIRFilter(signal, b) : applyIIRFilter(signal, b, a));
      ...
      const result = {
        ...
        output: {
          filtered: filtered.slice(0, 30).concat(filtered.length > 30 ? ['...'] : []),
    ...
    if (operation === 'frequency_response') {
      ...
      const result = {
        ...
        magnitude_dB: response.magnitudeDB.filter((_, i) => i % 8 === 0).map(m => Math.round(m * 100) / 100),
        phase_rad: response.phase.filter((_, i) => i % 8 === 0).map(p => Math.round(p * 100) / 100),
        summary: {
          dcGain_dB: Math.round(response.magnitudeDB[0] * 100) / 100,
          nyquistGain_dB: Math.round(response.magnitudeDB[response.magnitudeDB.length - 1] * 100) / 100,
    ...
    if (operation === 'analyze') {
      ...
      if (inputB && inputA) {
        ...
        return { toolCallId: id, content: 'Error: b and a coefficients required for analyze', isError: true };
      }
      ...
      const impulseResponse = a.length === 1
        ? applyFIRFilter(impulse, b)
        : applyIIRFilter(impulse, b, a);
      ...
      const result = {
        operation: 'analyze',
        coefficients: {
          b: b.slice(0, 10).concat(b.length > 10 ? ['...'] : []),
          a: a.slice(0, 10).concat(a.length > 10 ? ['...'] : []),
        ...
        frequencyResponse: {
          dcGain_dB: Math.round(response.magnitudeDB[0] * 100) / 100,
          nyquistGain_dB: Math.round(response.magnitudeDB[response.magnitudeDB.length - 1] * 100) / 100,
          passbandRipple_dB: Math.max(...response.magnitudeDB) - Math.min(...response.magnitudeDB.filter(m => m > -3))
        },
        impulseResponse: {
