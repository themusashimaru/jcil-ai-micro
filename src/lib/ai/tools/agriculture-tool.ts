...
export function accumulatedGDD(dailyTemps: Array<{ max: number; min: number }>, Tbase: number): number {
...
export function applicationEfficiency(waterApplied: number, waterStored: number): number {
...
export function nutrientRemoval(yield_kg_ha: number, nutrientContent: number): number {
...
export function bulkDensity(ovenDryWeight: number, volume: number): number {
  return ovenDryWeight / volume;
}

export function porosity(bulkDensity: number, particleDensity: number = 2.65): number {
