...
export const reverbTool: UnifiedTool = {
  ...
  parameters: {
    ...
    properties: {
      ...
      roomDimensions: {
        type: 'object',
        description: 'Room dimensions: { length: number, width: number, height: number }',
      },
      impulseResponse: {
        type: 'array',
        items: { type: 'number' },
...
function schroederReverb(
  ...
  const allpassFilters = allpassDelays.map((delay) => createAllpassFilter(delay, 0.5));
...
function freeverbReverb(
  ...
  // Create filters
  const combFiltersL = scaledCombL.map((delay) => createCombFilter(delay, feedback, damping));
  const combFiltersR = scaledCombR.map((delay) => createCombFilter(delay, feedback, damping));
  const allpassFiltersL = scaledAllpassL.map((delay) => createAllpassFilter(delay, 0.5));
  const allpassFiltersR = scaledAllpassR.map((delay) => createAllpassFilter(delay, 0.5));
...
function simulateRoom(
  ...
  const avgAbsorption =
    (wallArea * aWalls + floorArea * aFloor + ceilingArea * aCeiling) / surfaceArea;
