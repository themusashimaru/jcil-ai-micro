...
function executeRISCVInstruction(state: CPUState, instr: Instruction): ExecutionResult {
  ...
  switch (opcode) {
    case 'R-type':
      if (funct7 === 0x00) {
        switch (funct3) {
          case 0: setReg(rd!, getReg(rs1!) + getReg(rs2!)); break; // ADD
          case 1: setReg(rd!, getReg(rs1!) << (getReg(rs2!) & 0x1F)); break; // SLL
          case 2: setReg(rd!, (getReg(rs1!) | 0) < (getReg(rs2!) | 0) ? 1 : 0); break; // SLT
          case 3: setReg(rd!, getReg(rs1!) < getReg(rs2!) ? 1 : 0); break; // SLTU
          case 4: setReg(rd!, getReg(rs1!) ^ getReg(rs2!)); break; // XOR
          case 5: setReg(rd!, getReg(rs1!) >>> (getReg(rs2!) & 0x1F)); break; // SRL
          case 6: setReg(rd!, getReg(rs1!) | getReg(rs2!)); break; // OR
          case 7: setReg(rd!, getReg(rs1!) & getReg(rs2!)); break; // AND
        }
      } else if (funct7 === 0x20) {
        switch (funct3) {
          case 0: setReg(rd!, getReg(rs1!) - getReg(rs2!)); break; // SUB
          case 5: setReg(rd!, (getReg(rs1!) | 0) >> (getReg(rs2!) & 0x1F)); break; // SRA
        }
      } else if (funct7 === 0x01) {
        // M extension (multiply/divide)
        switch (funct3) {
          case 0: setReg(rd!, Math.imul(getReg(rs1!), getReg(rs2!))); break; // MUL
          case 4: { // DIV
    ...
    case 'I-type':
      switch (funct3) {
        case 0: setReg(rd!, getReg(rs1!) + imm!); break; // ADDI
        case 1: setReg(rd!, getReg(rs1!) << (imm! & 0x1F)); break; // SLLI
        case 2: setReg(rd!, (getReg(rs1!) | 0) < (imm! | 0) ? 1 : 0); break; // SLTI
        case 3: setReg(rd!, getReg(rs1!) < (imm! >>> 0) ? 1 : 0); break; // SLTIU
        case 4: setReg(rd!, getReg(rs1!) ^ imm!); break; // XORI
        case 5:
          if (funct7 === 0) setReg(rd!, getReg(rs1!) >>> (imm! & 0x1F)); // SRLI
          else setReg(rd!, (getReg(rs1!) | 0) >> (imm! & 0x1F)); // SRAI
          break;
        case 6: setReg(rd!, getReg(rs1!) | imm!); break; // ORI
        case 7: setReg(rd!, getReg(rs1!) & imm!); break; // ANDI
      }
      break;
    ...
    case 'Branch': {
      ...
      switch (funct3) {
        ...
        case 6: taken = au < bu; break; // BLTU
        case 7: taken = au >= bu; break; // BGEU
      }
...
function disassembleMIPS(instr: Instruction): string {
  ...
  if (opcode === 'R-type') {
    const functs: Record<number, string> = {
      ...
      0x2A: 'slt', 0x2B: 'sltu', 0x00: 'sll', 0x02: 'srl', 0x03: 'sra',
      0x08: 'jr', 0x09: 'jalr'
    };
  ...
  const iops: Record<number, string> = {
    ...
    0x23: 'lw', 0x2B: 'sw', 0x20: 'lb', 0x24: 'lbu',
  ...
  for (const line of lines) {
    ...
    switch (op) {
      case 'add':
        ...
        break;
      case 'sub':
        word = 0x33 | (regNum(parts[1]) << 7) | (regNum(parts[2]) << 15) | (regNum(parts[3]) << 20) | (0x20 << 25);
        break;
      case 'addi':
        word = 0x13 | (regNum(parts[1]) << 7) | (regNum(parts[2]) << 15) | ((parseInt(parts[3]) & 0xFFF) << 20);
        break;
      case 'lw': {
        const match = parts[2].match(/(-?\d+)\((\w+)\)/);
        if (match) {
          const offset = parseInt(match[1]) & 0xFFF;
          word = 0x03 | (2 << 12) | (regNum(parts[1]) << 7) | (regNum(match[2]) << 15) | (offset << 20);
        }
        break;
      }
      case 'sw': {
        ...
        if (match) {
          ...
          word = 0x23 | (2 << 12) | ((offset & 0x1F) << 7) | (regNum(match[2]) << 15) | (regNum(parts[1]) << 20) | (((offset >> 5) & 0x7F) << 25);
        }
        break;
      }
      case 'beq': {
...
export const cpusimulatorTool: UnifiedTool = {
  name: 'cpu_simulator',
  description: 'Simulate CPU architectures - RISC-V, MIPS instruction execution, pipeline, disassembly',
  parameters: {
    type: 'object',
    properties: {
      operation: {
        type: 'string',
        enum: ['execute', 'step', 'run', 'disassemble', 'assemble', 'registers', 'memory', 'pipeline', 'sample', 'info'],
        ...
        if (operation === 'step') {
          ...
          return {
            ...
            content: JSON.stringify({
              ...
              pc: state.pc,
              halted: state.halted,
              registers: RISCV_REGISTER_NAMES.reduce((acc, name, i) => {
          ...
          content: JSON.stringify({
            ...
            registers: RISCV_REGISTER_NAMES.reduce((acc, name, i) => {
              if (state.registers[i] !== 0) {
                acc[name] = { decimal: state.registers[i], hex: '0x' + state.registers[i].toString(16) };
              }
              return acc;
      ...
      case 'registers': {
        ...
        return {
          ...
          content: JSON.stringify({
            ...
            specialRegisters: arch === 'RISC-V'
              ? { pc: 'Program Counter', zero: 'Hardwired zero', ra: 'Return address', sp: 'Stack pointer' }
              : { pc: 'Program Counter', hi: 'High multiply result', lo: 'Low multiply result' }
          }, null, 2)
        };
      }
      ...
      case 'sample': {
        ...
        return {
          ...
          content: JSON.stringify({
            ...
            assembly: sample.assembly.trim().split('\n').filter((l: string) => l.trim()),
          ...
          content: JSON.stringify({
            ...
            supportedArchitectures: {
              'RISC-V': {
                ...
                features: ['R-type', 'I-type', 'S-type', 'B-type', 'U-type', 'J-type instructions'],
            ...
            pipelineStages: ['IF (Fetch)', 'ID (Decode)', 'EX (Execute)', 'MEM (Memory)', 'WB (Writeback)']
          }, null, 2)
        };
      }
    }
