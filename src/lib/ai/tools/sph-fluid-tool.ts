...
function getNeighborCellKeys(pos: Vector3, cellSize: number): string[] {
  ...
  for (let dx = -1; dx <= 1; dx++)
    for (let dy = -1; dy <= 1; dy++)
      for (let dz = -1; dz <= 1; dz++) keys.push(`${cx + dx},${cy + dy},${cz + dz}`);
  return keys;
}
...
function computePressureForces(state: SPHState): void {
  ...
  for (const pi of particles) {
    ...
    for (const j of pi.neighbors) {
      ...
      force = vec3Add(
        force,
        vec3Scale(gradKernelSpiky(rVec, r, h), -pi.mass * pj.mass * pressureTerm)
      );
    }
  ...
  for (const pi of particles) {
    ...
    for (const j of pi.neighbors) {
      ...
      force = vec3Add(
        ...
        vec3Scale(
          velDiff,
          (config.viscosity * pj.mass * laplacianKernelViscosity(r, h)) / pj.density
        )
      );
    }
    pi.acceleration = vec3Add(pi.acceleration, force);
...
function computeSurfaceTensionForces(state: SPHState): void {
  ...
  for (const pi of particles) {
    ...
    for (const j of pi.neighbors) {
      ...
      colorGradient = vec3Add(
        colorGradient,
        vec3Scale(gradKernelPoly6(rVec, r, h), pj.mass / pj.density)
      );
    }
  ...
  for (const pi of particles) {
    ...
    pi.acceleration = vec3Add(
      pi.acceleration,
      vec3Scale(normal, -config.surfaceTension * colorLaplacian)
    );
  }
}
...
function handleBoundaries(state: SPHState): void {
  ...
  for (const p of particles) {
    ...
    } else if (p.position.x > bounds.max.x) {
    ...
    } else if (p.position.y > bounds.max.y) {
    ...
    } else if (p.position.z > bounds.max.z) {
...
function initializeSPH(
  ...
  const boundaryParticles = createBoundaryParticles(
    { min: domainMin, max: domainMax },
    spacing,
    config,
    fluidParticles.length
  );
  return {
...
function simulateSPH(params: {
  ...
  for (let step = 0; step < steps; step++) {
    ...
    if (step % recordInterval === 0) {
      ...
      const avgDensity =
        fluidParticles.reduce((sum, p) => sum + p.density, 0) / fluidParticles.length;
...
export const sphfluidTool: UnifiedTool = {
  name: 'sph_fluid',
  description:
    'Smoothed Particle Hydrodynamics (SPH) fluid simulation. Features density computation, pressure forces, viscosity forces, surface tension, boundary handling, and efficient neighbor search.',
  parameters: {
    type: 'object',
    properties: {
      operation: {
      ...
      fluidMin: {
        type: 'object',
        description: 'Fluid volume min corner: { x: number, y: number, z: number }',
      },
      fluidMax: {
        type: 'object',
        description: 'Fluid volume max corner: { x: number, y: number, z: number }',
      },
      domainMin: {
        type: 'object',
        description: 'Domain min corner: { x: number, y: number, z: number }',
      },
      domainMax: {
        type: 'object',
        description: 'Domain max corner: { x: number, y: number, z: number }',
      },
      config: {
        type: 'object',
        description:
          'SPH config: { restDensity, particleMass, smoothingRadius, stiffness, viscosity, surfaceTension, gravity: {x,y,z}, boundaryDamping }',
      },
  ...
  try {
    ...
    switch (operation) {
      ...
      case 'analyze': {
        ...
        const densityVar =
          fluid.reduce((s, p) => s + Math.pow(p.density - avgDensity, 2), 0) / fluid.length;
        result = {
          ...
          incompressibilityError:
            (
              (Math.abs(avgDensity - (args.config?.restDensity || 1000)) /
                (args.config?.restDensity || 1000)) *
              100
            ).toFixed(2) + '%',
        };
        break;
      ...
      default:
        result = {
          ...
          equations: {
            ...
            pressure: 'p = k(ρ - ρ₀)',
            pressureForce: 'F = -Σⱼ mⱼ(pᵢ/ρᵢ² + pⱼ/ρⱼ²)∇W',
          },
...
export function issphfluidAvailable(): boolean {
  return true;
}
