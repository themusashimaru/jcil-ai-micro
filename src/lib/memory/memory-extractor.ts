 ...
 */
function formatConversationForAnalysis(messages: Array<{ role: string; content: string }>): string {
  return messages
...
function isValidFact(fact: unknown): fact is ExtractedFact {
  ...
  return typeof f.category === 'string' && typeof f.fact === 'string' && f.fact.length > 0;
}
...
function sanitizeCategory(category: string): ExtractedFact['category'] {
  ...
  return allowed.includes(normalized) ? (normalized as ExtractedFact['category']) : 'other';
}
 ...
 */
export function extractTopicsLocally(messages: Array<{ role: string; content: string }>): string[] {
