...
When calling composio_GMAIL_SEND_EMAIL, use these exact parameter names:
- \`recipient_email\`: The recipient's email address (required)
- \`subject\`: Email subject line (required)
- \`body\`: Email body text, supports HTML (required)
- \`cc\`: CC recipients (comma-separated emails)
- \`bcc\`: BCC recipients (comma-separated emails)
- \`attachment_urls\`: Array of URLs to attach (for files generated by create_document or other tools)

### Sending Emails with Attachments
When the user asks to create a document AND email it:
1. First generate the document using create_document — it returns a downloadUrl
2. Then use the downloadUrl as an attachment in composio_GMAIL_SEND_EMAIL:
   \`"attachment_urls": ["https://...the-download-url-from-create-document..."]\`
3. Always mention the attachment in the email body so the recipient knows to look for it
...
\`\`\`action-preview
{
  ...
  "toolParams": { "recipient_email": "...", "subject": "...", "body": "...", "attachment_urls": ["https://..."] }
}
\`\`\`
