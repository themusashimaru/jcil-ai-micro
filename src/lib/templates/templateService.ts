...
export async function getTemplateByCategory(
  ...
  try {
    const { data, error } = await untypedFrom(supabase, 'website_templates')
      .select('*')
...
export async function incrementTemplateUsage(templateId: string): Promise<void> {
  ...
  try {
    ...
    await (supabase.rpc as CallableFunction)('increment_template_usage', {
...
export function applyBusinessInfo(
  ...
  if (generatedImages?.logo) {
    ...
    html = html.replace(
      /src=["'](?:logo\.png|placeholder|#logo|\/logo)[^"']*["']/gi,
      `src="${generatedImages.logo}"`
    );
  }
  if (generatedImages?.hero) {
    ...
    html = html.replace(
      /src=["'](?:hero\.jpg|hero\.png|placeholder|#hero|\/hero)[^"']*["']/gi,
      `src="${generatedImages.hero}"`
    );
    html = html.replace(
      /background-image:\s*url\(["']?(?:hero\.jpg|hero\.png|placeholder|#hero|\/hero)[^)]*\)/gi,
      `background-image: url('${generatedImages.hero}')`
    );
  }
...
export async function saveGeneratedSite(
  ...
  try {
    const { data, error } = await untypedFrom(supabase, 'generated_sites')
      .insert({
        user_id: userId,
        template_id: templateId,
