 ...
 *
 * PURPOSE:
 * - Provide Grok AI chat completion functionality
 * - Support streaming responses
 * - OpenAI-compatible API (xAI uses OpenAI SDK format)
 *
 * FEATURES:
 * - Dual-pool round-robin API key system (same as Anthropic/OpenAI/Perplexity)
 * - Dynamic key detection (XAI_API_KEY_1, _2, _3, ... unlimited)
 * - Fallback pool (XAI_API_KEY_FALLBACK_1, _2, ... unlimited)
...
// Default model: Grok 3 Mini (balanced cost/performance)
const DEFAULT_MODEL: XAIModel = 'grok-3-mini';
...
// ========================================
// Primary Pool: Round-robin load distribution (XAI_API_KEY_1, _2, _3, ... unlimited)
// Fallback Pool: Emergency reserve (XAI_API_KEY_FALLBACK_1, _2, _3, ... unlimited)
// Backward Compatible: Single XAI_API_KEY still works
...
/**
 * Initialize all available xAI API keys into their pools
...
function initializeApiKeys(): void {
  ...
  // If no numbered keys found, fall back to single XAI_API_KEY
  if (primaryPool.length === 0) {
    const singleKey = process.env.XAI_API_KEY;
    if (singleKey) {
      primaryPool.push({
        key: singleKey,
        rateLimitedUntil: 0,
  ...
  let j = 1;
  while (true) {
    const key = process.env[`XAI_API_KEY_FALLBACK_${j}`];
    if (!key) break; // Stop when we hit a gap
  ...
  if (totalKeys > 0) {
    console.log(`[xAI] Initialized dual-pool system (dynamic detection):`);
    console.log(`[xAI]   Primary pool: ${primaryPool.length} key(s) (round-robin load distribution)`);
    console.log(`[xAI]   Fallback pool: ${fallbackPool.length} key(s) (emergency reserve)`);
  }
}
...
function getFallbackKeyState(): ApiKeyState | null {
  ...
  for (let i = 0; i < fallbackPool.length; i++) {
    ...
    if (keyState.rateLimitedUntil <= now) {
      ...
      console.log(`[xAI] Using FALLBACK key ${keyState.index} (primary pool exhausted)`);
      return keyState;
    }
  }
...
function getApiKeyState(): ApiKeyState | null {
  ...
  console.log(`[xAI] All ${allKeys.length} keys rate limited. Using ${soonestKey.pool} key ${soonestKey.index} (available in ${waitTime}s)`);
 ...
 */
export function markXAIKeyRateLimited(apiKey: string, retryAfterSeconds: number = 60): void {
  const allKeys = [...primaryPool, ...fallbackPool];
  const keyState = allKeys.find(k => k.key === apiKey);

  if (keyState) {
    keyState.rateLimitedUntil = Date.now() + (retryAfterSeconds * 1000);
    console.log(`[xAI] ${keyState.pool.toUpperCase()} key ${keyState.index} rate limited for ${retryAfterSeconds}s`);
    ...
    console.log(`[xAI] Pool status: ${availablePrimary}/${primaryPool.length} primary, ${availableFallback}/${fallbackPool.length} fallback available`);
  }
}
 ...
 */
export function getXAITotalKeyCount(): number {
  initializeApiKeys();
  return primaryPool.length + fallbackPool.length;
}

/**
 * Check if xAI is configured (has at least one API key)
 */
 ...
 */
export function getXAIKeyStats(): {
  primaryKeys: number;
 ...
 */
export function getCurrentXAIApiKey(): string | null {
  const keyState = getApiKeyState();
  return keyState?.key || null;
}

/**
 * Get xAI client for current key (with caching)
...
function getXAIClient(): OpenAI {
  ...
  if (!keyState) {
    throw new Error('XAI_API_KEY is not configured. Set XAI_API_KEY or XAI_API_KEY_1, _2, etc.');
  }
 ...
 */
export async function createXAICompletion(options: XAIChatOptions): Promise<{
  text: string;
  model: XAIModel;
}> {
  const {
  ...
  const currentKey = getCurrentXAIApiKey();
  ...
  console.log('[xAI] Creating completion with model:', model, 'stream:', stream);
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      if (attempt > 0) {
        console.log(`[xAI] Retry attempt ${attempt + 1}/${maxRetries}`);
      }

      if (stream) {
        // Streaming mode - return a stream
      ...
      if (isRateLimit && currentKey) {
        ...
        markXAIKeyRateLimited(currentKey, retryAfter);

        if (attempt < maxRetries - 1) {
          console.log(`[xAI] Rate limited, trying next key...`);
          continue;
        }
      }
      ...
      if (attempt === maxRetries - 1) {
        console.error('[xAI] Chat completion error (all retries exhausted):', lastError);
        throw lastError;
      }

      console.error(`[xAI] Error on attempt ${attempt + 1}, retrying:`, lastError.message);
    }
  }

  throw lastError || new Error('All xAI API keys exhausted');
}

/**
 * Create a streaming chat completion using Grok
 */
export async function createXAIStreamingCompletion(options: XAIChatOptions): Promise<{
  ...
  console.log('[xAI] Creating streaming completion with model:', model);
  ...
  // Create a TransformStream to convert xAI stream to text stream
