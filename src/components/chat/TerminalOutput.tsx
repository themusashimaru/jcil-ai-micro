...
function detectOutputType(output: string): 'error' | 'success' | 'warning' | 'info' | 'normal' {
  ...
  // Warning patterns
  if (lowerOutput.includes('warning:') || lowerOutput.includes('warn:') || lowerOutput.includes('deprecated')) {
    return 'warning';
  }
  ...
  return (
    <div
      ...
      style={{
        ...
        borderColor: success ? '#30363d' : '#f85149',
      }}
    >
      {/* Header */}
      <div
        ...
        style={{
          backgroundColor: success ? 'rgba(56, 139, 253, 0.1)' : 'rgba(248, 81, 73, 0.1)',
          borderColor: '#30363d',
        }}
      >
        <div className="flex items-center gap-2">
          ...
          {outputType === 'warning' && (
            <span className="text-xs px-1.5 py-0.5 rounded bg-yellow-500/20 text-yellow-400">Warning</span>
          )}
          {outputType === 'success' && (
            <span className="text-xs px-1.5 py-0.5 rounded bg-green-500/20 text-green-400">Success</span>
          )}
        </div>
      ...
      {command && (
        <div
          className="px-3 py-2 font-mono text-sm border-b flex items-center gap-2"
      ...
      <div className="px-3 py-2 overflow-x-auto">
        <pre className="text-sm font-mono leading-relaxed" style={{ margin: 0 }}>
          {displayLines.map((line, i) => (
            <div key={i} className="flex">
              {showLineNumbers && (
                <span
                  className="select-none pr-3 text-right min-w-[3ch]"
                  style={{ color: '#484f58' }}
                >
                  {i + 1}
                </span>
...
export function CodeDiffOutput({ filename, additions = [], deletions = [], context: _context = [] }: CodeDiffOutputProps) {
  return (
    <div className="my-3 rounded-lg overflow-hidden border" style={{ backgroundColor: '#0d1117', borderColor: '#30363d' }}>
      {/* Header */}
      <div
      ...
      >
        ...
        {deletions.length > 0 && (
          <span className="text-xs text-red-400">-{deletions.length}</span>
        )}
      </div>
      ...
      <div className="px-3 py-2 font-mono text-sm">
        {deletions.map((line, i) => (
          <div key={`del-${i}`} className="flex" style={{ backgroundColor: 'rgba(248, 81, 73, 0.1)' }}>
            <span className="text-red-400 pr-2">-</span>
            <span className="text-red-300">{line}</span>
          </div>
        ))}
        {additions.map((line, i) => (
          <div key={`add-${i}`} className="flex" style={{ backgroundColor: 'rgba(63, 185, 80, 0.1)' }}>
