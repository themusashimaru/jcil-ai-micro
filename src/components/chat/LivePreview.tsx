    ...
    try {
      ...
      if (!ComponentToRender) {
        const lastDefinedComponent = (() => {
          ...
          ${
            code
              .match(/(?:function|const|let|var)\s+([A-Z][a-zA-Z0-9]*)/g)
              ?.map((m) => {
...
export function LivePreview({
  ...
  return (
    <div className="my-3 rounded-lg overflow-hidden border bg-[#0d1117] border-[#30363d]">
      {/* Header */}
      <div className="flex items-center justify-between px-3 py-2 border-b bg-green-500/10 border-[#30363d]">
            ...
            {Object.entries(DEVICE_PRESETS).map(([key, value]) => (
              <button
                ...
                className={`px-2 py-1 text-xs rounded ${
                  device === key ? 'bg-gray-700 text-white' : 'text-gray-400 hover:text-gray-200'
                }`}
              >
                {value.label}
              </button>
      ...
      {/* Footer */}
      <div className="px-3 py-1.5 text-xs text-gray-500 border-t bg-[#161b22] border-[#30363d]">
