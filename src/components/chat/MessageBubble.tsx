...
export function MessageBubble({ message, isLast }: MessageBubbleProps) {
  ...
  return (
    <div className={`flex items-start gap-3 ${isUser ? 'justify-end' : ''}`}>
      ...
      {!isUser && (
        <div className="mt-1 flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full bg-white/5 text-gray-400">
      ...
      {/* Message Content */}
      <div className={`group space-y-2 ${isUser ? 'max-w-[70%]' : 'flex-1'}`}>
        {/* Tool Calls */}
        ...
        {/* Message Bubble */}
        <div
          className={`chat-bubble chat-bubble-tail relative ${
        ...
        >
          ...
          {!isUser && (
            <button
              onClick={handleCopy}
              className="absolute right-2 top-2 rounded p-1.5 text-gray-400 opacity-0 transition-opacity hover:bg-white/10 hover:text-white group-hover:opacity-100"
