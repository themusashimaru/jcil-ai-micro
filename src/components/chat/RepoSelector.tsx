...
export function RepoSelector({ isOpen, onClose, onSelect }: RepoSelectorProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      {/* Modal */}
      <div className="relative w-full max-w-lg mx-4 rounded-xl overflow-hidden shadow-2xl bg-glass border border-theme">
        {/* Header */}
        ...
        <div className="max-h-80 overflow-y-auto">
          {!githubConnected ? (
            <div className="px-4 py-8 text-center">
              ...
              <p className="text-sm text-text-muted">Connect your GitHub account to push code</p>
              <p className="text-xs mt-2 text-text-muted">Go to Settings → Connectors → GitHub</p>
            </div>
          ) : loadingRepos ? (
            <div className="px-4 py-8 text-center">
            ...
            <div className="py-2">
              {filteredRepos.map((repo) => (
                ...
                >
                  <RepoIcon className="w-5 h-5 mt-0.5 flex-shrink-0" isPrivate={repo.private} />
                  <div className="flex-1 min-w-0">
                    ...
                    {repo.description && (
                      <p className="text-xs mt-0.5 truncate text-text-muted">{repo.description}</p>
                    )}
                    <div className="text-xs mt-1 flex items-center gap-2 text-text-muted">
                      <span>{repo.owner}</span>
                      <span>•</span>
                      <span>{repo.defaultBranch}</span>
                    </div>
                  </div>
                  <ChevronRightIcon className="w-5 h-5 flex-shrink-0 opacity-50 text-text-muted" />
                </button>
              ))}
            </div>
          )}
        ...
        <div className="px-4 py-3 border-t border-theme flex items-center justify-between">
          ...
          <button
            ...
            className="text-xs px-3 py-1.5 rounded-lg hover:bg-white/10 transition-colors disabled:opacity-50 text-primary"
          >
            Refresh
          </button>
        </div>
...
function GitHubIcon({ className }: { className?: string }) {
  return (
    <svg className={`${className} text-text-primary`} viewBox="0 0 24 24" fill="currentColor">
...
function RepoIcon({ className, isPrivate }: { className?: string; isPrivate?: boolean }) {
  return (
    <svg
      ...
      viewBox="0 0 16 16"
      fill="currentColor"
      style={{ color: isPrivate ? '#ffc107' : 'var(--text-muted)' }}
    >
